...
"\n"
"options:\n"
"-stdev <init_stdev>: set init stdev for parameters\n"
...
Option parse_option(int argc, char **argv) {
    ...
    for (; i < argc; i++) {
        ...
        } else if (args[i].compare("-stdev") == 0) {
            if (i == argc-1)
                throw invalid_argument("need to specify init_stdev after -stdev");
            i++;
            opt.param.stdev = atof(args[i].c_str());
            if (opt.param.stdev <= 0)
                throw invalid_argument("init stdev should be greater than zero");
...
int train_on_disk(Option opt) {
    ...
    fm_read_problem_to_disk(opt.tr_path, tr_bin_path);
